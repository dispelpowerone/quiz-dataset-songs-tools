...
def download_track(track_link: str, file_name: str):
    subprocess.run(
        [
            f"curl -vvv --max-time 60 --retry 3 --output {file_name} -H @headers.txt {track_link}"
        ],
        shell=True,
    )
...
def retrieve_one_song(
    title: str, artist: str, track_file: str, src_dir: str, dest_dir: str
) -> dict[str, str | None]:
...
index = retrieve_from_list(
    songs_list, "input/popular_youtube_2", "output/popular_youtube_2/tracks"
)
